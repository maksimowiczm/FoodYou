...
class RecipeRepositoryIntegrationTest {
    ...
    @BeforeTest
    fun setup() {
        database = provideRoomDatabaseBuilder<RecipeDatabase>().buildDatabase()
        eventBus = ListEventBus()
        repository = RecipeRepositoryImpl(database = database, integrationEventBus = eventBus)
    }
    ...
    fun delete_publishesRecipeDeletedEvent() = runTest {
        ...
        val createResult =
            repository.create(
                ...
                name = RecipeName("Recipe to Delete"),
                servings = 1.0,
                image = null,
                note = null,
                finalWeight = null,
                ingredients =
                    listOf(
                        RecipeIngredient(
                            foodReference = FoodReference.UserFood("user-food-1"),
                            quantity = AbsoluteQuantity.Weight(Grams(100.0)),
                        )
                    ),
            )
        val identity = assertIs<Result.Success<RecipeIdentity, *>>(createResult).data
        ...
        val event = assertIs<RecipeDeletedEvent>(publishedEvents[0])
    ...
    fun delete_recipeWithIngredients_cascadesDelete() = runTest {
        ...
        val createResult =
            repository.create(
                accountId = testAccountId,
                name = RecipeName("Recipe with Ingredients"),
                servings = 1.0,
                image = null,
                note = null,
                finalWeight = null,
                ingredients =
                    listOf(
                        RecipeIngredient(
                            foodReference = FoodReference.UserFood("food-1"),
                            quantity = AbsoluteQuantity.Weight(Grams(100.0)),
                        ),
                        RecipeIngredient(
                            foodReference = FoodReference.UserFood("food-2"),
    ...
    fun update_replacesIngredients() = runTest {
        ...
        val createResult =
            repository.create(
                accountId = testAccountId,
                name = RecipeName("Original Recipe"),
                servings = 1.0,
                image = null,
                note = null,
                finalWeight = null,
        ...
        val updateResult =
            repository.update(
                identity = identity,
                name = RecipeName("Updated Recipe"),
                servings = 2.0,
                image = null,
                note = null,
                finalWeight = null,
                ingredients =
                    listOf(
                        ...
                        RecipeIngredient(
                            foodReference = FoodReference.UserFood("food-3"),
    ...
    fun update_withCircularReference_rollsBackTransaction() = runTest {
        ...
        val recipeAId = assertIs<Result.Success<RecipeIdentity, *>>(recipeAResult).data
        ...
        val updateResult =
            repository.update(
                identity = recipeAId,
                name = RecipeName("Recipe A Updated"),
                servings = 2.0,
                image = null,
                note = null,
                finalWeight = null,
                ingredients =
                    listOf(
                        RecipeIngredient(
                            foodReference = FoodReference.Recipe(recipeAId.id),
                            quantity = AbsoluteQuantity.Weight(Grams(100.0)),
                        )
                    ),
            )
        ...
        val error = assertIs<Result.Error<*, CircularRecipeReferenceError>>(updateResult).error
    ...
    fun update_withIndirectCircularReference_detectsCycleAndRollsBack() = runTest {
        ...
        val recipeBId = assertIs<Result.Success<RecipeIdentity, *>>(recipeBResult).data
        ...
        val updateResult =
            repository.update(
                ...
                ingredients =
                    listOf(
                        RecipeIngredient(
                            foodReference = FoodReference.Recipe(recipeBId.id),
                            quantity = AbsoluteQuantity.Weight(Grams(100.0)),
                        )
                    ),
            )
        ...
        val _ = assertIs<Result.Error<*, CircularRecipeReferenceError>>(updateResult)
        ...
        assertEquals(
            recipeAId.id,
            (recipeB.ingredients[0].foodReference as FoodReference.Recipe).foodId,
        )
    }
    ...
    fun findRecipesUsingFood_detectsDependencies() = runTest {
        ...
        val baseRecipeId = assertIs<Result.Success<RecipeIdentity, *>>(baseRecipeResult).data

        // Create recipes that depend on base recipe
        repository.create(
            accountId = testAccountId,
            name = RecipeName("Dependent Recipe 1"),
            servings = 1.0,
            image = null,
            note = null,
            finalWeight = null,
            ingredients =
                listOf(
                    RecipeIngredient(
                        foodReference = FoodReference.Recipe(baseRecipeId.id),
                        quantity = AbsoluteQuantity.Weight(Grams(50.0)),
                    )
                ),
        )
        ...
        repository.create(
            accountId = testAccountId,
            name = RecipeName("Dependent Recipe 2"),
            servings = 1.0,
            image = null,
            note = null,
            finalWeight = null,
        ...
        repository.create(
            accountId = testAccountId,
            name = RecipeName("Independent Recipe"),
            servings = 1.0,
            image = null,
            note = null,
            finalWeight = null,
    ...
    fun findRecipesUsingFood_withUserFoodReference_findsAllDependencies() = runTest {
        ...
        repository.create(
            ...
            ingredients =
                listOf(
                    RecipeIngredient(
                        foodReference = FoodReference.UserFood(targetFoodId),
                        quantity = AbsoluteQuantity.Weight(Grams(100.0)),
                    )
                ),
        )
        ...
        repository.create(
            ...
            ingredients =
                listOf(
                    ...
                    RecipeIngredient(
                        foodReference = FoodReference.UserFood("other-food"),
                        quantity = AbsoluteQuantity.Weight(Grams(50.0)),
                    ),
                ),
        )
        ...
        repository.create(
            ...
            ingredients =
                listOf(
                    RecipeIngredient(
                        foodReference = FoodReference.UserFood("different-food"),
                        quantity = AbsoluteQuantity.Weight(Grams(100.0)),
                    )
                ),
        )
    ...
    fun delete_withNestedRecipeReferences_maintainsDataIntegrity() = runTest {
        ...
        val recipeCId = assertIs<Result.Success<RecipeIdentity, *>>(recipeCResult).data

        // Verify dependency chain
        val recipesUsingA =
            repository.findRecipesUsingFood(FoodReference.Recipe(recipeAId.id), testAccountId)
        assertEquals(1, recipesUsingA.size)
        assertEquals("Recipe B", recipesUsingA[0].name.value)

        val recipesUsingB =
            repository.findRecipesUsingFood(FoodReference.Recipe(recipeBId.id), testAccountId)
        ...
        assertEquals(
            recipeBId.id,
            (recipeC.ingredients[0].foodReference as FoodReference.Recipe).foodId,
        )

        // Event was published
        val events = eventBus.publishedEvents.filterIsInstance<RecipeDeletedEvent>()
    ...
    fun accountIsolation_recipesAreAccountSpecific() = runTest {
        ...
        val recipe1Result =
            repository.create(
                accountId = account1,
                name = RecipeName("Account 1 Recipe"),
                servings = 1.0,
                image = null,
                note = null,
                finalWeight = null,
        ...
        val recipe1Id = assertIs<Result.Success<RecipeIdentity, *>>(recipe1Result).data

        val recipe2Result =
            repository.create(
                accountId = account2,
                name = RecipeName("Account 2 Recipe"),
                servings = 1.0,
                image = null,
                note = null,
                finalWeight = null,
        ...
        val recipe2Id = assertIs<Result.Success<RecipeIdentity, *>>(recipe2Result).data
        ...
        val account1Recipes =
            repository.findRecipesUsingFood(FoodReference.UserFood("food-2"), account1)
        assertEquals(0, account1Recipes.size)

        val account2Recipes =
            repository.findRecipesUsingFood(FoodReference.UserFood("food-1"), account2)
