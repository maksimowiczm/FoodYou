...
@Composable
private fun ContributorsRow(modifier: Modifier = Modifier) {
    val listState = rememberLazyListState()
    val density = LocalDensity.current
    val speedPxPerSecond = with(density) { 40.dp.toPx() }
    ...
    LaunchedEffect(Unit) {
        ...
        while (true) {
            ...
            last = now
            listState.scrollBy(speedPxPerSecond * deltaSeconds)
        }
    }
    ...
    Column(
        ...
        Text(
            ...
            style = MaterialTheme.typography.brand.titleLarge.copy(brush = brush),
        )
        Spacer(Modifier.height(8.dp))
        LazyRow(
            state = listState,
            ...
            items(count = Int.MAX_VALUE) { i ->
                Text(
                    text = shuffledContributors[i % shuffledContributors.size],
