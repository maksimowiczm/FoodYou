...
internal class ImportCsvProductsUseCaseImpl(
        ...
        csvLines.chunked(CHUNK_SIZE).collect { line ->
            val importedProducts =
                line
                    ...
                    .map { csvLine -> csvLine.lineToProduct(parser, fieldOrder) }
                    .map { productMap ->
                        val entity =
                            mapper.toProduct(
                        ...
                        entity.copy(sourceUrl = source?.url ?: entity.sourceUrl)
                    }
                    .map {
                        ...
                        createProductUseCase.createUnique(
                            ...
                            note = model.note,
                            source = FoodSource(type = it.sourceType, url = it.sourceUrl),
    ...
    override suspend fun importWithFeedback(
        ...
        csvLines.chunked(CHUNK_SIZE).map { lines -> import(fieldOrder, lines.asFlow(), source) }
    ...
    private fun String.lineToProduct(
        ...
        val map = fieldOrder.zip(split).associate { (field, value) -> field to value?.trim() }
