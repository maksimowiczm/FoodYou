...
sealed interface NutrientValue {
    ...
    operator fun times(other: Float): NutrientValue =
        when (this) {
            is Complete -> Complete(value * other)
            is Incomplete -> Incomplete(value?.times(other))
        }

    operator fun div(other: Float): NutrientValue =
        when (this) {
            is Complete -> Complete(value / other)
...
fun List<NutrientValue>.sum(): NutrientValue =
    this.fold<NutrientValue, NutrientValue>(NutrientValue.Complete(0f)) { acc, nutrientValue ->
        acc + nutrientValue
    }
...
object NutrientValueAsNullableFloatSerializer : KSerializer<NutrientValue> {
    override val descriptor: SerialDescriptor =
        PrimitiveSerialDescriptor(serialName = "NutrientValue", kind = PrimitiveKind.FLOAT).nullable
    ...
    override fun deserialize(decoder: Decoder): NutrientValue =
