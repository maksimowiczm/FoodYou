...
/**
 * Command to export products to a CSV file.
 *
 * @property fields A list of product fields to include in the CSV export.
 *
 * Results in a flow of strings representing the CSV content.
 */
data class ExportCsvProductsCommand(val fields: List<ProductField>) : Command<Flow<String>, Unit>

internal class ExportCsvProductsCommandHandler(
    private val transactionProvider: DatabaseTransactionProvider,
    private val localProduct: LocalProductDataSource,
) : CommandHandler<ExportCsvProductsCommand, Flow<String>, Unit> {

    override suspend fun handle(command: ExportCsvProductsCommand): Result<Flow<String>, Unit> =
        channelFlow {
                ...
                transactionProvider.withTransaction {
                    var offset = 0
                    while (true) {
                        val products = localProduct.observeProducts(PAGE_SIZE, offset).first()
                        if (products.isEmpty()) break

                        for (product in products) {
                            val csvLine =
                                command.fields.joinToString(separator = ",") { field ->
