...
/**
 * Returns the first [LoadState.Error] from the [CombinedLoadStates] or `null` if there are no errors.
 * If there are multiple errors, the first one encountered in the order of refresh, prepend, and append
 */
val CombinedLoadStates.error: Throwable?
    get() = when (val refresh = refresh) {
